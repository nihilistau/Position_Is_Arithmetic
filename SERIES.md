...
| — | XBAR — the Auditable Latent Crossbar (probe P1: inter-model memory through latent state, no tokens) | 12B steered by direct KV-cache transplant: 15/15 incorporation, 15/15 selectivity (double dissociation), 3.69 orders max rank pull, gold-instrument coherence | **ledger row X-R1 citable** (architecture: lattice `RFC-XBAR-auditable-latent-crossbar.md`); paper not yet staged | when the lane has a standalone repro |
