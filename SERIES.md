...
## Papers 25–27 — the memory agency, tools, and the heartbeat (KEYSTONE, 2026-06-25)

The KEYSTONE milestone (lattice `papers/PPT-LAT-KEYSTONE.md`): the pieces lock into one self-curating organism — the model owns its memory, acts through tools, and tidies its own state between turns. All default-off (byte-identical null floor); host-side, no frozen-ABI / `.sp-model` change.

- **25 — The memory agency** (`X-AGENCY`): the model forgets, supersedes, and merges its own facts. FORGET (`SP_FORGET`, token-overlap match → drop from live set + rewrite the persisted registry), DECIDE (`SP_DECIDE`, the model answers `CHANGED=n` to supersede, gated by the "cannot both be true at once" test), MERGE (the model returns one combined fact; the daemon drops both originals + captures the synthesis). Emergent self-curation — the verdict is the model's, not a dedup rule. Gates G-FORGET / G-DECIDE / G-MERGE.
- **26 — Ephemeral tool calling** (`X-HARNESS-TOOLS`): tool calling + sandboxed Python on a **text-only** backend. The model emits `<tool name="…">{json}</tool>` in plain text; `run_with_tools` parses + executes + feeds back (ReAct); `ToolSpec.from_callable` derives the schema from a Python signature. CosySim's runtime re-hosted on sp-daemon (lmstudio stripped). Honest negative: multi-line *indented* code in a JSON string is unreliable (one-liners clean; the model saw its own stderr and retried). Gates G-HARNESS-DAEMON-E2E (H1) / G-HARNESS-TOOLCALL-E2E (H2).
- **27 — The agency loop** (`X-AGENCY-LOOP`): the model does memory work **between turns**, not just stop. An agency round shows the model its own memory + the curation tools and lets it decide what to forget/consolidate; the KAIROS scheduler fires it on a heartbeat, **idle-gated** so it never starves a live turn. The "auto rounds" become where the organism acts. Gates G-HARNESS-MEMTOOLS-E2E (H3) / G-HARNESS-AGENCY-E2E (H4).

## Papers 28–30 — tiered memory, faithfulness, and the capstone (KEYSTONE, 2026-06-25)

The memory model, the sharp grounding lesson, and the integration that locks the whole series under one stone. All default-off (byte-identical null floor); host-side, no frozen-ABI / `.sp-model` change.

- **28 — Tiered memory: short, mid, and long on one content-addressed store** (`X-CONVMEM`): three memory tiers on the MEM-OKF store — SHORT = the live conversation (re-prefilled; a planted name survives 20 turns), MID = durable facts extracted into the recall registry, LONG = the whole conversation stored **complete** (`full/`) **and** **summarized** (`sum/`) under one **sha256** address (gist by default via `recall_conversations`, verbatim on demand via `read_conversation`); three disclosure tiers LUT → `sum/` → `full/`; plus the init priming (system prompt + recallable capabilities corpus + non-parametric seed facts). Gates G-HARNESS-CONVMEM-E2E / G-HARNESS-CONSOLIDATE / G-HARNESS-HOOK-E2E.
- **29 — Conversational faithfulness: the chat didn't restart, it confabulated** (`X-FAITHFUL`): a sharp lesson — the "restarts each turn" feeling was a **misdiagnosis** (the daemon carries the full thread; a planted name survives, recall on and off). The real failure was **faithfulness** — the model confabulated an earlier stated fact (said "dog" over the user's "octopus"), leaning on parametric priors over grounding. Fix: a default system prompt ("never substitute a stated fact") → `FAITHFUL=True` (engine `88d924e`). Meta-lesson banked: served-model misbehavior is almost always **ours** (template / decode / sampler / forward / prompt), not the weights — verify vs llama.cpp + our PPL first (the EOT companion case blamed the quant twice, wrongly; `SP_EOT_BIAS`, engine `9e4b40f`). The structural answer is the tiered memory of paper 28; the prompt is the patch.
- **30 — KEYSTONE: the night the arches locked into one organism** (`X-KEYSTONE`) — **THE CAPSTONE**: the integration as the result. The isolated proofs (byte-exact forward 19–21, two-ring/integer memory 13–18, the learned librarian 24, the diffusion judge) lock into **one self-supporting organism** at milestone **keystone-1** — faithful conversation + memory agency (store/forget/decide/merge) + tool calling + Python + the agency heartbeat + tiered conversation memory + self-priming, **all on the byte-exact `O_K` substrate, all default-off = null floor, all receipted**, the loop closing in zero manual steps. ~90% built; open edges named (O(1) conversation KV via L1 §6b, the 2-physical-GPU byte-exact check, deeper faithfulness, native-C XBAR + T4 weights). The container is exact; the structure is learned. The foundation to build forward on.
