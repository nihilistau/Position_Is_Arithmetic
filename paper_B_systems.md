...
The Möbius step decomposes each row of $K$ into a sum over square-free divisors with $\mu$-weighted coefficients. The square-free step stores only the square-free-indexed coefficients (≈60.8% of indices for vocabularies in the range 32K–128K). The spinor step applies a fixed unit element of $\mathcal{O}_K^\times$ that rotates the basis into a representation aligned to hardware-favored bit boundaries.
...
This is on phone CPU only — no GPU, NPU, or HTP offload. Commit `shannon-prime-llama@05c405d`. Spec-decode contributes the majority of the speedup at this size class; the SP fast path provides a further ~30% on top via cache-resident FUSED_KQ.
...
Predicted outcome: $|\Delta\mathrm{PPL}| < 1\%$ for SP-fp8 with zero calibration; standard fp8 methods require ~1000 calibration samples to reach the same accuracy.
...
Train a small (≤1B param) model from scratch with the embedding initialized as Hecke eigenforms of $S_k(\Gamma_0(N))$ for $\dim = d_{\mathrm{embed}}$. Compare to learned embedding at fixed compute. This is the longest-path experiment.
