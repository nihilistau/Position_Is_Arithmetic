...
Differential testing against a reference implementation is standard practice; golden-model verification is the norm in hardware design, where RTL is graded against a slow, trusted behavioral model before tape-out. The position here is the software-inference transposition of that norm, with two specifics: the grading currency is distributional (argmax + KL + teacher-forcing) rather than bitwise-at-every-node, because norm layers make raw activation error a lying witness (§3, paper 05); and the golden model must be *rebuilt from the artifact* when the ecosystem reference is suspect (§6), because a frozen calibration inherits the health of the thing it was calibrated against.

## 10. Receipts

This paper rests on the following ledger rows ([`LEDGER.md`](../../LEDGER.md)), each carrying its config, gate, and caveat:

- **04-R1** — full variable-geometry forward == oracle: argmax 12/12, max KL 2.663e-10, |Δlogit| 1.84e-4.
- **04-R2** — autoregressive decode == oracle, teacher-forced: all 12 generated tokens predicted.
- **04-R3** — first-try composition: both live runs green on first attempt after five staged gates, 38/38 cumulative.
- **04-R4** — oracle arithmetic enforced: per-weight dequant diverges 2.8e-3; the inline lift restores the floor.
- **06-R8** — the gemma4 GGUF ecosystem ships broken weights; the hand-written reference (4.68) exposed it; forward exonerated, artifacts convicted.
- **06-R9** — the sovereign safetensors-direct pipeline reproduces ground truth: OK_Q8 4.7396 (+1.33%), B1 5.1259 (sim-predicted to four decimals).
- **06-R10** — the honest 12B point: 26.1 tok/s at PPL 5.12 on a 12 GB card, under triple-instrument agreement (5.1259 / 5.1259 / 5.1160).

Engine provenance: `tests/test_gemma4_cuda.c`, tag `stage-eta-phase1-closed-2026-06-06`. Gold instruments: lattice `tests/gemma4_gold/` (`_t2_manual_forward.py`, `_t2b_gguf_forensics.py`, `_t2c_gold_on_gguf.py`, `_t2h_spmodel_gate.py`); full receipt trail in the lattice contract `papers/CONTRACT-SPEED-wire-tok-s.md` (Addendum and Resolution of 2026-06-07, Artifact Gate and Closed-Green of 2026-06-08). Companion: paper 05, the probe suite this methodology rides on.
