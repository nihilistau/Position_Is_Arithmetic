...
### 5.5 The TurboQuant Connection: Cartesian Decomposition

A recent independent result provides structural validation of the lattice PE framework. TurboQuant (Google Research, ICLR 2026) achieves near-optimal KV cache compression through a two-stage process: PolarQuant converts vectors from Cartesian to polar coordinates (separating magnitude from direction), then QJL corrects the residual error via 1-bit Johnson-Lindenstrauss sketching.

PolarQuant implements the Archimedean projection. The KV cache in Cartesian coordinates is "the sphere" — all dimensions are entangled through normalisation. PolarQuant projects this onto "the cylinder" — a Cartesian product of independent magnitude × direction components. This removes the dependence between components, making per-coordinate quantization near-optimal.

The connection to lattice PE is structural. RoPE encodes position as rotation of Q and K vectors at frequency-dependent angular rates. Attention scores depend on angular differences between rotated vectors. PolarQuant preserves angular relationships by construction. But geometric RoPE creates *entangled* angular rates (all derived from powers of 10,000), so PolarQuant's random rotation must disentangle them before quantization. Lattice PE creates *approximately independent* angular rates (coprime periods don't alias), with the tier structure already separating rates by scale. The heavy lifting of scale separation is already done by the PE itself.
...
This also provides a mechanistic explanation for the previously speculative quantization resilience claim: the advantage is not that "logical structure survives quantization better than precision" (hand-waving), but that **pre-factored angular structure produces smaller PolarQuant residuals** (a concrete, measurable prediction).
...
- **TurboQuant alignment test — PARTIALLY CONFIRMED.** The prediction that lattice PE KV caches show structural asymmetry between K (spectral concentration) and V (uniform spectrum) is now empirically confirmed at the level of WHT banded quantization (Section 5.5). The stronger claim — that lattice PE produces smaller PolarQuant residuals than geometric PE at equivalent bit-width — remains untested and is the next planned experiment.
- **Combined PrimePE + VHT2.** Running SpectralRoPEALiBi PE *together* with VHT2 KV compression (rather than testing each independently) is the key integration experiment. Theory predicts the combination should outperform either alone.
...
- Zandieh et al. (2024) — Quantized Johnson-Lindenstrauss Transform (QJL)
- Hsu et al. (2026) — TurboQuant: Online Vector Quantization with Near-optimal Distortion Rate (ICLR 2026)
...
Systematic sweep validating Section 5.5 predictions. All runs: WikiText-2 test set, ctx=2048, batch=512, chunks=4 (±0.43 PPL noise floor).
...
1. **sk = head_dim always.** WHT requires the full vector. sk=32 on head_dim=64 → PPL +47%.
2. **3-bit floor.** 2-bit on any band is catastrophic (V:4/2 → PPL +1.59%).
3. **n=4 optimal for K.** More bands add scale overhead; n=5 and n=8 are within noise but cost 14% compression.
4. **Flat beats banded for V.** No exceptions in the sweep.
