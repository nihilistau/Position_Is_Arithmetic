...
only the square-free-indexed coefficients (≈60.8\% of indices for
vocabularies in the range 32K--128K). The spinor step applies a fixed
unit element of \(\mathcal{O}_K^\times\) that rotates the basis into a
representation aligned to hardware-favored bit boundaries.
...
further \textasciitilde30\% on top via cache-resident FUSED\_KQ.
...
calibration; standard fp8 methods require \textasciitilde1000
calibration samples to reach the same accuracy.
...
Train a small (≤1B param) model from scratch with the embedding
initialized as Hecke eigenforms of \(S_k(\Gamma_0(N))\) for
\(\dim = d_{\mathrm{embed}}\). Compare to learned embedding at fixed
compute. This is the longest-path experiment.
