...
def collect(body):
    agent.agent_chat_stream = fake_stream  # monkeypatch the streaming agent
    app.agent_chat_stream = fake_stream
    # app imports agent_chat_stream inside the fn, so patch the source module
    import sys as _s
    _s.modules["harness.agent"].agent_chat_stream = fake_stream
    events = []
